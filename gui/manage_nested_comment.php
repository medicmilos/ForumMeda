...
<?php    
		if(isset($_REQUEST['btnSaveUprofile'])){
			...
			$upit = "UPDATE nested_comments SET username = '".$username."', comment = '".$comment."'  WHERE id_nested_comments = '".$_REQUEST['id']."'";
	...
	$upit = "SELECT * FROM nested_comments WHERE id_nested_comments='".$_REQUEST['id']."'";
			include("konekcija.php");
			$rezultat = mysql_query($upit, $konekcija);  
			mysql_close($konekcija);
		...
		$pom = 0;
		echo("<form action='index.php?page=17' method='GET'>");
